...
describe('registry config', async () => {
  ...
  it('should update registry', async () => {
    const destPath = './test/fixtures/registry/output'
    ...
    registry.update(destPath)

    expect(existsSync(join(destPath, 'registry.json'))).toBe(true)

    const content = readFileSync(join(destPath, 'registry.json'), 'utf-8')

    expect(JSON.parse(content)).toMatchInlineSnapshot(`
      {
        "$schema": "https://weme-ui.github.io/schema/registry.json",
        "access": "public",
        "description": "The basic test registry for Weme UI.",
