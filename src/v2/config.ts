 ...
 */
export const ConfigRegistry = z.object({
  /**
   * Repository name/URL.
   *
  ...
  default: z.boolean().optional().meta({
    ...
    description: 'Whether this registry is the default registry, which will be used when the registry is not specified in the component usage.',
  }),
}).meta({
 ...
 */
export const ConfigPaths = z.object({
...
export const ConfigSchemaV2 = z.object({
   ...
   */
  $schema: z.url().trim().default(CONFIG_SCHEMA_URL).meta({
  ...
  description: 'This schema defines the structure of the project config JSON file used in Weme UI.',
})
